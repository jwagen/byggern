...
//Initialize functions related to the board
void board_init(){
	adc_init();
	DDRH |= (1<<PH0);
	PORTH |= (1<<PH0);
...
//Check if ball has broken beam
//Includes a simple filter, that ensures that the signal is valid for at least 3 calls of this function or 60ms.
...
//Pulse the solinoid
//Delay based, should perhaps be interupt based
//Could be done in the interupt function that runs every 20 ms.
