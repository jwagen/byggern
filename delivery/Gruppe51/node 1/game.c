/*
 * game.c
 * This is the main game section of the system. When a game is started it reads input data and sends it to node2
 * Node 2 wil send back the status of the ball, so this game functions can count the score.
 *
...
//Stored in eeprom, for non volatile storage
uint8_t EEMEM game_top_scores_rom[7];

//Control mode
game_control_mode_t game_control_mode = JOYSTICK;

//Initialize the systems the game needs
//Not all systems are initialised here, as many of them are shared with functions that have already called them.
void game_init(){
...
void game_run(){
	...
	//Only run if game is started, and it is not game over
	if(game_started && !game_status){
		...
		//Update the display with new point information
		game_update_display();
		
	}	
}

//Start the game, with set initial values
void game_start(){
	printf("Game started\n");
	game_started = 1;
	game_life_counter = 3;
...
void game_new_score(uint8_t score){
	...
	//Check if score is larger than smallest currently in memory, and put it in if it is
