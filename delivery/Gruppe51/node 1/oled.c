...
//Holds the current cursor posision 
...
//Moves the cursor to specified line
...
//Moves cursor to spesified column
...
//Data written to display while oled_inverted_state is 1 gets inverted
void oled_inverted(uint8_t inv){
	oled_inverted_state = inv;
}

//Write a singe character to the display
...
//Write a whole bitmap directly to the display.
void oled_display_bitmap(const unsigned char* bitmap){
	oled_pos(0,0);
	for(uint8_t y = 0; y<8; y++){
		for (uint8_t x = 0;x < 128; x++){
			//Reverse bit order, due to format of the generated bitmaps
...
//Send display data from ram to oled
